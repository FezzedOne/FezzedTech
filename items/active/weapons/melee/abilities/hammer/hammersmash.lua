...
function HammerSmash:windup(windupProgress)
  ...
  if windupProgress >= 1.0 then
    if math.abs(world.gravity(mcontroller.position())) > 0 and (mcontroller.groundMovement() or not self.groundOnly) then
...
function HammerSmash:fire()
  ...
  -- Fixed a bug where the smashMomentum vec2 isn't copied before modification.
  local smashMomentum = { self.smashMomentum[1], self.smashMomentum[2] }
