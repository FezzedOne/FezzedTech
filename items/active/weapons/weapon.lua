...
function Weapon:new(weaponConfig)
  ...
  newWeapon.damageLevelMultiplier = config.getParameter("damageLevelMultiplier",
    root.evalFunction("weaponDamageLevelMultiplier", config.getParameter("level", 1)))
  newWeapon.elementalType = config.getParameter("elementalType")
  newWeapon.jumpFireMode = config.getParameter("jumpFireMode") or nil
  newWeapon.muzzleOffset = config.getParameter("muzzleOffset") or { 0, 0 }
  newWeapon.aimOffset = config.getParameter("aimOffset") or
  (newWeapon.muzzleOffset[2] - 0.25)                                                           -- why is it off by 0.25? nobody knows!
  newWeapon.abilities = {}
...
function Weapon:addTransformationGroup(name, offset, rotation, rotationCenter)
  ...
  table.insert(self.transformationGroups,
    { name = name, offset = offset, rotation = rotation, rotationCenter = rotationCenter })
end
...
function dwDisallowFlip()
  if self and self.weapon and self.weapon.stance then
...
function dwIsInvisible()
  if self and self.weapon and self.weapon.stance then
    return self.weapon.stance.invisible
  end
...
function dwIsStable()
  if self and self.weapon and self.weapon.stance then
    return self.weapon.stance.isStable
  end
...
function dwCanFlyWithItem()
  if self and self.weapon and self.weapon.stance then
    return self.weapon.stance.canFlyWithItem or false
  end
