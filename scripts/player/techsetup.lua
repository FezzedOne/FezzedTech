...
function init()
    if xsb or starExtensions then
        message.setHandler(
          "/roll", function(_, sameClient, rawArgs)
              if sameClient then
                  local args = {}
                  local helpMessage = "^gray;Format is ^cyan;/roll <dice> [is public] [comment]^gray;. The comment does not need to be quoted. The ^cyan;<is public>^gray; argument may be skipped before the comment.^reset;"
                  if starExtensions then
                      helpMessage = "^gray;Format is ^cyan;/roll <dice> [is public] [comment]^gray;. Surround the comment with quotes. The ^cyan;<is public>^gray; argument may be skipped before the comment.^reset;"
                  ...
                  if die then
                      ...
                      if noErr and total then
                          local rollMessage
                          if comment then
                              rollMessage = "<< Rolled ^orange;" .. tostring(total) .. "^reset; on ^cyan;" .. die .. "^reset;. // " .. comment .. "^reset; >>"
                          else
                              rollMessage = "<< Rolled ^orange;" .. tostring(total) .. "^reset; on ^cyan;" .. die .. "^reset;. >>"
                          end

                          if public then
                              local sendMode = (public == "local" or public == true) and "Local" or (public == "party" and "Party" or "Broadcast")
                              if starExtensions then
        ...
        message.setHandler(
          "/rab", function(_, sameClient, rawArgs)
              if sameClient then
                  local args = {}
                  local helpMessage = "^gray;Format is ^cyan;/rab <number of rolls> <dice> [is public] [comment]^gray;. The comment does not need to be quoted. The ^cyan;<is public>^gray; argument may be skipped before the comment.^reset;"
                  if starExtensions then
                      helpMessage = "^gray;Format is ^cyan;/rab <number of rolls> <dice> [is public] [comment]^gray;. Surround the comment with quotes. The ^cyan;<is public>^gray; argument may be skipped before the comment.^reset;"
                      args = table.pack(chat.parseArguments(rawArgs))
                  else
                      for arg in rawArgs:gmatch("%S+") do
                          sb.logInfo("arg = \"%s\"", arg)
                  ...
                  if public then
                      ...
                      if (not starExtensions) and args[5] then for i = 5, #args do comment = comment .. " " .. args[i] end end
                  else
                      comment = args[3] and tostring(args[3])
                      if (not starExtensions) and args[4] then for i = 4, #args do comment = comment .. " " .. args[i] end end
                  end
                  ...
                  if die and rolls >= 1 then
                      ...
                      if not nilTotal then
                          ...
                          local rollMessage
                          if comment then
                              rollMessage = "<< " .. diceNumberMessage .. " Results: " .. tostring(totalStr) .. " // " .. comment .. "^reset; >>"
                          else
        ...
        message.setHandler(
          "/ra", function(_, sameClient, rawArgs)
              sb.logInfo("rawArgs = \"%s\"", rawArgs)
              if sameClient then
                  local args = {}
                  local helpMessage = "^gray;Format is ^cyan;/ra <skill/stat value> [is public] [comment]^gray;. Uses GURPS resolution. The comment does not need to be quoted. The ^cyan;<is public>^gray; argument may be skipped before the comment.^reset;"
                  if starExtensions then
                      helpMessage = "^gray;Format is ^cyan;/ra <skill/stat value> [is public] [comment]^gray;. Uses GURPS resolution. Surround the comment with quotes. The ^cyan;<is public>^gray; argument may be skipped before the comment.^reset;"
                      args = table.pack(chat.parseArguments(rawArgs))
                  else
                      for arg in rawArgs:gmatch("%S+") do
                          sb.logInfo("arg = \"%s\"", arg)
                  ...
                  if targetValue then
                      ...
                      if noErr and dieRoll then
                          ...
                          if dieRoll >= critFailureThreshold then
                              marginStr = "Result is ^orange;" .. tostring(dieRoll) .. "^reset;. ^#b00;Critical failure!^reset;"
                          elseif dieRoll <= critSuccessThreshold then
                              marginStr = "Result is ^orange;" .. tostring(dieRoll) .. "^reset;. ^#dd4;Critical success!^reset;"
                          else
                              if margin >= 0 then
                                  marginStr = "Result is ^orange;" .. tostring(dieRoll) .. "^reset;. ^green;Succeeded by " .. tostring(absMargin) .. ".^reset;"
                              else
                                  marginStr = "Result is ^orange;" .. tostring(dieRoll) .. "^reset;. ^red;Failed by " .. tostring(absMargin) .. ".^reset;"
                              end
                          end

                          local rollMessage
                          if comment then
                              rollMessage = "<< Rolled against ^cyan;" .. tostring(targetValue) .. "^reset;. " .. tostring(marginStr) .. " // " .. comment ..
                                              "^reset; >>"
                          else
                              rollMessage = "<< Rolled against ^cyan;" .. tostring(targetValue) .. "^reset;. " .. tostring(marginStr) .. " >>"
                          end
        ...
        message.setHandler(
          "/raba", function(_, sameClient, rawArgs)
              if sameClient then
                  local args = {}
                  local helpMessage = "^gray;Format is ^cyan;/raba <number of rolls> <skill/stat value> [is public] [comment]^gray;. Uses GURPS resolution. The comment does not need to be quoted. The ^cyan;<is public>^gray; argument may be skipped before the comment.^reset;"
                  if starExtensions then
                      helpMessage = "^gray;Format is ^cyan;/raba <number of rolls> <skill/stat value> [is public] [comment]^gray;. Uses GURPS resolution. Surround the comment with quotes. The ^cyan;<is public>^gray; argument may be skipped before the comment.^reset;"
                      args = table.pack(chat.parseArguments(rawArgs))
                  else
                      for arg in rawArgs:gmatch("%S+") do
                          sb.logInfo("arg = \"%s\"", arg)
                  ...
                  if targetValue and (rolls >= 1) then
                      ...
                      if not nilMargin then
                          ...
                          if rolls == 1 then
                              ...
                              rollNumberMessage = "Rolled " .. tostring(rolls) .. " times against ^cyan;" .. targetValue .. "^reset;."
                          end
                          local rollMessage
                          if comment then
                              rollMessage = "<< " .. rollNumberMessage .. " Results: " .. tostring(marginStr) .. " // " .. comment .. "^reset; >>"
                          else
    ...
    if xsb then -- xSB xClient detected.
        ...
        function player.setHairDirectives(newDirectives) player.setIdentity({hairDirectives = tostring(newDirectives) or ""}) end

        function player.bodyDirectives() return player.getIdentity().bodyDirectives end

        function player.setBodyDirectives(newDirectives) player.setIdentity({bodyDirectives = tostring(newDirectives) or ""}) end

        function player.emoteDirectives() return player.getIdentity().emoteDirectives end

        function player.setEmoteDirectives(newDirectives) player.setIdentity({emoteDirectives = tostring(newDirectives) or ""}) end
    ...
    if player.species() ~= "FutaraDragon" then
        ...
        if (not player.equippedTech("Head")) or player.equippedTech("Head") == "rptech" then player.equipTech("emptyheadtech") end
    ...
    self.lastLegless = (status.statPositive("legless") or status.statusProperty("legless")) and (not status.statusProperty("ignoreFezzedTechAppearance"))
...
function update()
    ...
    local legless = (status.statPositive("legless") or status.statusProperty("legless")) and (not status.statusProperty("ignoreFezzedTechAppearance"))
    local grandfatheredLeglessChar = (status.statPositive("isLeglessChar") or status.statusProperty("isLeglessChar")) and
                                       (not status.statusProperty("ignoreFezzedTechAppearance"))

    if player.setBodyDirectives and (not grandfatheredLeglessChar) then
        if legless and not self.lastLegless then
            if player.imagePath() == "floran" or player.imagePath() == "human" or
              ((player.species() == "floran" or player.species() == "human") and player.imagePath() == nil) then
                ...
                local isHuman = baseImagePath == "human" or (player.species() == "human" and baseImagePath == nil) or savedImagePath == "human"
