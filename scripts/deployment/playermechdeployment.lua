...
function init()
    ...
    message.setHandler("getMechItemSet", function() return self.itemSet end)
...
function update(dt)
    if self.deployTicks then
        ...
        if self.deployTicks <= 0 then
            ...
            elseif player.isDeployed() then
                deploy()
                if storage.vehicleId then world.sendEntityMessage(storage.vehicleId, "deploy") end
    ...
    if storage.vehicleId and world.entityType(storage.vehicleId) ~= "vehicle" then storage.vehicleId = nil end

    if self.beaconCheck and self.beaconCheck:finished() then
        if self.beaconCheck:succeeded() then self.beaconPosition = self.beaconCheck:result() end
        self.beaconCheck = nil
    end

    if storage.vehicleId then
        if not self.energyCheck then self.energyCheck = world.sendEntityMessage(storage.vehicleId, "currentEnergy") end

        if self.energyCheck and self.energyCheck:finished() then
            if self.energyCheck:succeeded() then self.mechEnergyRatio = self.energyCheck:result() end
...
function teleportOut() despawnMech() end

function canDeploy() return not not self.mechParameters end
